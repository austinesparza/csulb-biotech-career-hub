...
if (!/create\s+table(?:\s+if\s+not\s+exists)?\s+public\.discovery_feedback\b/i.test(sql)
    || !/create\s+table(?:\s+if\s+not\s+exists)?\s+public\.discovery_model_versions\b/i.test(sql)
    || !/create\s+table(?:\s+if\s+not\s+exists)?\s+public\.discovery_lead_predictions\b/i.test(sql)) {
  failures.push('private discovery learning tables are missing');
}
if (!/create\s+or\s+replace\s+function\s+public\.record_discovery_feedback/i.test(sql)
