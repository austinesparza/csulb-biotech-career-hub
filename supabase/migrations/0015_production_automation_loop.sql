...
comment on function public.schedule_due_source_fetch_runs(integer) is
  'Queues due governed sources exactly once while an active run exists. Does not publish.';
comment on function public.queue_pipeline_health_tasks() is
  'Creates deduplicated officer tasks for unhealthy sources and stale public records. Does not publish.';
