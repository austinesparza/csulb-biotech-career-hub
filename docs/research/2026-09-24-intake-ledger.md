...
The draft PR separates the 49 source-research rows from ordinary student submissions, sorts them by research priority, and asks an officer to attest to an individual employer/ATS posting before creating another private draft. It also removes the bulk lead promotion button and disables its server action. A recorded employer URL is not proof of an open, graduate-accessible posting. These interface safeguards take effect only when the PR is merged and deployed.
