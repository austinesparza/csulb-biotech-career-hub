...
| `jobs[n].updated_at` | `sourceUpdatedAt` | Strict RFC3339 validation; canonicalized to UTC ISO (`toISOString`); invalid values become null + `source_updated_at_invalid` |
...
An absolute operation deadline is enforced with `deadlineMs = Date.now() + timeoutMs`.
Deadline checks run:

1. Before JSON parsing
2. Immediately after JSON parsing
3. During record normalization
4. Before returning success

If the deadline is exceeded at any check, the connector returns:
...
JSON parsing itself is synchronous and bounded by `maxResponseBytes` because the
raw response is fully size-limited before parsing.
...
block, and an absolute deadline check runs before JSON parsing, after JSON
parsing, during normalization, and before returning success.
