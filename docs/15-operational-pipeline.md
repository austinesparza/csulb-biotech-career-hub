...
The September 11 public Greenhouse check included Xaira Therapeutics, Ginkgo Bioworks, and Flagship Pioneering's co-op program. A live connector check observed 52 records and retained all 52 for private archival. Score version 3 routed 11 scientifically relevant internships and co-ops toward review and held unrelated or full-time roles below the review threshold. This check did not write to production. The Flagship co-op source was later removed from the starter cohort because its board is restricted to Northeastern University students.
...
A follow-up September 25 audit found seven draft co-ops and 23 pending tasks from that restricted board still in the private review queue. The source-policy cleanup archives only the seven pending drafts, dismisses the 23 tasks with a reason, and retains prior officer decisions and fetched evidence. A database constraint prevents enabling the restricted Greenhouse token through either source identifier or board-token override. Other Flagship companies' independent career boards remain eligible for evaluation against their own published requirements. The candidate scores on those archived rows measure scientific relevance, not access for a CSULB student.
