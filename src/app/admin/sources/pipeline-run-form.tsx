  ...
  return <div className="mt-4 space-y-3">
    ...
    >
      ...
      {typeof state.claimed === 'number' ? <p className="mt-1 text-xs">
        ...
        {state.claimed} runs processed · {state.completed ?? 0} completed · {state.failed ?? 0} failed ·{' '}
        {state.recordsSeen ?? 0} postings observed · {state.repaired ?? 0} bridge gaps repaired ·{' '}
