...
const supportingSentence = (rawText: string, quoteContains: string): string | null => {
  const lower = rawText.toLocaleLowerCase();
  const needle = quoteContains.toLocaleLowerCase();
  const index = lower.indexOf(needle);
  if (index < 0) return null;

  // Split only on real sentence/newline boundaries. A bare `.` is not safe
  // because compensation and GPA evidence commonly contains decimal points.
  const priorNewline = rawText.lastIndexOf("\n", index);
  const priorSentence = rawText.lastIndexOf(". ", index);
  const start = Math.max(priorNewline, priorSentence < 0 ? -1 : priorSentence + 1) + 1;
  const after = index + quoteContains.length;
  const nextNewline = rawText.indexOf("\n", after);
  const nextSentence = rawText.indexOf(". ", after);
  const ends = [nextNewline, nextSentence < 0 ? -1 : nextSentence + 1].filter((value) => value >= 0);
  const end = ends.length ? Math.min(...ends) : rawText.length;
...
const perfect = (c: any) => {
  ...
  for (const [field, exp] of Object.entries<any>(c.expected)) {
    const quote = exp.quoteContains ? supportingSentence(c.rawText, exp.quoteContains) : null;
...
console.log("=== Golden-set structure ===\n");
{
  const lanes = new Set(golden.cases.map((c: any) => c.lane));
  const labelledFields = new Set(golden.cases.flatMap((c: any) => Object.keys(c.expected)));
  ok("at least 30 labelled cases", golden.cases.length >= 30, String(golden.cases.length));
  ok("all ten scientific lanes represented", lanes.size === 10, [...lanes].join(","));
  ok("every extraction field has a positive example", FIELDS.every((field) => labelledFields.has(field)), FIELDS.filter((field) => !labelledFields.has(field)).join(","));
  ok("case IDs are unique", new Set(golden.cases.map((c: any) => c.id)).size === golden.cases.length);
  ok("corpus is explicitly labelled synthetic", golden.corpusClass === "synthetic-contract");
}
...
console.log("\n=== Fabricating model is caught ===\n");
{
  ...
  ok("fabrication surfaces as unbound (quote not in source)", report.overall.bindingFailures === golden.cases.length, String(report.overall.bindingFailures));
...
{
  const results = golden.cases.flatMap((c: any) => scoreCase(c, perfect(c), FIELDS));
  const text = formatReport(aggregate(results, FIELDS, { model: "perfect", promptVersion: "v1", cases: golden.cases.length }));
...
console.log("\n=== Decision-flip error rate ===\n");
{
  ...
  const payReport = aggregate(payWrong, FIELDS, { model: "pay-off-by-one", promptVersion: "v1", cases: golden.cases.length });
  ...
  const eligReport = aggregate(eligLost, FIELDS, { model: "drops-citizenship", promptVersion: "v1", cases: golden.cases.length });
  ...
  const dReport = aggregate(deadlineLost, FIELDS, { model: "drops-deadline", promptVersion: "v1", cases: golden.cases.length });
  ...
  const cleanReport = aggregate(clean, FIELDS, { model: "perfect", promptVersion: "v1", cases: golden.cases.length });
